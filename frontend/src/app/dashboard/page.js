...
export default function Dashboard() {
    ...
    if (user) {
      setActiveTab(
        user.role === 'ADMIN' ? 'reports' :
          user.role === 'RECEPTIONIST' ? 'patients' : 'appointments'
      );
    }
  }, [user]);
  ...
  const searchPhysiciansAdmin = async () => {
    try {
      // PROPERLY ENCODE USER INPUT BEFORE SENDING THE SEARCH QUERY //
      const encodedQuery = encodeURIComponent(adminSearchQuery.trim());
      const res = await fetch(`${API_BASE_URL}/doctors?search=${encodedQuery}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (Array.isArray(data)) {
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {doctorAppointments.map((app) => (
                        <tr key={app.id} className="hover:bg-slate-500/5 transition-colors">
                          ...
                          <td className="py-3.5">
                            <Link
                              href={`/patients/${app.patient?.id}/history-records`}
