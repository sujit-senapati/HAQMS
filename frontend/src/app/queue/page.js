...
export default function QueueMonitor() {
  ...
  // REMOVED THE HARDCODED API_BASE_URL FROM HERE

  const fetchQueueData = async () => {
    try {
      // SINCE IT'S A PUBLIC DASHBOARD WHICH SHOWS QUEUES, I HAVE UPDATED CODE IN THE BACKEND //
      // SO IT DOES NOT ASK FOR AUTHENTICATION WHILE FECTHING QUEUE DATA //
      ...
      // UPDATED THE HARDCODED API DOMAIN WITH .env FILE URL SERVING //
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/queue`);
      if (!res.ok) {
  ...
  useEffect(() => {
    ...
    // TO FIX THE MEMORY LEAK CLEAR INTERVAL IS RETURNED //
    // NO DEPENDENCY ARRAY BECAUSE WE WANT IT TO RUN ONLY ONCE ON MOUNT //
